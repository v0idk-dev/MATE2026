...
def _scale_K_to_image(K, calib_size, image_size):
    """Scale camera matrix K from calibration resolution to actual image resolution."""
...
def stereo_calibrate():
    ...
    # Scale intrinsics if the per-camera calibration was done at a different
    # resolution than the stereo images (e.g. calibrated at 1280x720, stereo
    # captured at 640x360). Infer calibration resolution from cx/cy: the
    # principal point should be near the image centre.
    calib_w = int(round(K1[0, 2] * 2))
    calib_h = int(round(K1[1, 2] * 2))
    if (calib_w, calib_h) != left_size:
        scale_x = left_size[0] / calib_w
        scale_y = left_size[1] / calib_h
        print(f"  Scaling intrinsics from inferred {calib_w}x{calib_h} "
              f"→ {left_size[0]}x{left_size[1]} "
              f"(scale {scale_x:.4f}x, {scale_y:.4f}y)")
        K1 = _scale_K_to_image(K1, (calib_w, calib_h), left_size)
        K2 = _scale_K_to_image(K2, (calib_w, calib_h), left_size)
    ...
    if USE_FISHEYE:
        ...
        rms, K1_o, D1_o, K2_o, D2_o, R, T, _rvecs, _tvecs = cv2.fisheye.stereoCalibrate(
